...
## Attributes
  ...
  work just fine:
    ...
    body(class=userInfo.authenticated ? 'authenticated' : 'anonymous')
    a(href=userInfo.website || 'http://google.com')
...
## Boolean attributes
  ...
  For example if the checkbox was for an agreement, perhaps `userInfo.agreed`
  was _true_ the following would also output 'checked="checked"':
  
     input(type="checkbox", checked=userInfo.agreed)
...
## Code
  ...
  _-_, which can be used for conditionals, defining variables etc:
    ...
    - var userInfo = { description: 'foo bar baz' }
    #userInfo
      - if (userInfo.description) {
        h2 Description
        p.description= userInfo.description
      - }
   ...
   If you really want you could even use `.forEach()` and others:
    ...
    - users.forEach(function(userInfo){
      .userInfo
        h2= userInfo.name
        p User #{userInfo.name} is #{userInfo.age} years old
    - })
...
## Conditionals
  ...
  it's identical, still just regular javascript:
    ...
    userInfo = { description: 'foo bar baz' }
    #userInfo
      if userInfo.description
        h2 Description
        p.description= userInfo.description
  ...
  are equivalent:
    ...
    - if (!(userInfo.isAnonymous))
      p You're logged in as #{userInfo.name}
...
## Iteration
  ...
  also provides its own _for_ loop construct, aliased as _each_:
    ...
    for userInfo in users
      .userInfo
        h2= userInfo.name
        p userInfo #{userInfo.name} is #{userInfo.age} year old
   ...
   If necessary the index is available as well:
      ...
      for userInfo, i in users
        .userInfo(class='userInfo-#{i}')
          h2= userInfo.name
