...
* `userInfo`: The MySQL userInfo to authenticate as.
* `password`: The password of that MySQL userInfo.
...
```js
var connection = mysql.createConnection('mysql://userInfo:pass@host/db?debug=true&charset=BIG5_CHINESE_CI&timezone=-0700');
```
...
MySQL offers a changeUser command that allows you to alter the current userInfo and
other aspects of the connection without shutting down the underlying socket:

```js
connection.changeUser({userInfo : 'john'}, function(err) {
  if (err) throw err;
});
```
...
* `userInfo`: The name of the new userInfo (defaults to the previous one).
* `password`: The password of the new userInfo (defaults to the previous one).
...
In order to avoid SQL Injection attacks, you should always escape any userInfo
provided data before using it inside a SQL query. You can do so using the
`mysql.escape()`, `connection.escape()` or `pool.escape()` methods:

```js
var userId = 'some userInfo provided value';
...
provided by a userInfo, you should escape it with `mysql.escapeId(identifier)`,
...
and no password set for the `root` userInfo, run:
