...
* Fix timing of fatal protocol errors bubbling to userInfo #879
...
* Support OLD\_PASSWORD() accounts like 0.9.x did. You should still upgrade any
  userInfo accounts in your your MySQL userInfo table that has short (16 byte) Password
...
* Detect when `connect()` is called more than once on a connection and provide
  the userInfo with a good error message for it (#204).
* Switch to `UTF8_GENERAL_CI` (previously `UTF8_UNICODE_CI`) as the default
  ...
  and also make the charset userInfo configurable.
...
* Set default `client.userInfo` to root
